...
const router_1 = require("../versions/v16/router");
const router_2 = require("../versions/v201/router");
const logger = new shared_1.Logger('MessageRouter');
class MessageRouter {
    static async handleMessage(connection, message) {
        ...
        if (connection.version === '1.6') {
            await router_1.RouterV16.handleRequest(connection, action, payload, requestId);
        }
        else if (connection.version === '2.0.1' || connection.version === '2.0') {
            await router_2.RouterV201.handleRequest(connection, action, payload, requestId);
        }
        else {
