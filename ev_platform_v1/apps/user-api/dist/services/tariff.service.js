...
const logger = new shared_1.Logger('TariffService');
class TariffService {
    /**
     ...
     */
    static async calculateCost(energyKwh, durationMin, startTime, chargerId) {
        try {
            // Find Tariff
            const station = await shared_1.Charger.findOne({ charger_id: chargerId });
            let tariff = null;
            if (station && station.tariff_id) {
                tariff = await shared_1.Tariff.findById(station.tariff_id);
            }
            // Fallback to Site Tariff
            if (!tariff && station && station.site_id) {
                const site = await shared_1.Site.findById(station.site_id);
                if (site && site.tariff_id) {
                    tariff = await shared_1.Tariff.findById(site.tariff_id);
                }
            }
            if (!tariff) {
                // Default Fallback
    ...
    static isPeakHour(time, peakHours) {
