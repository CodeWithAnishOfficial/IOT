...
const logger = new shared_1.Logger('SearchController');
    ...
    try {
        ...
        const nearby = stations.map((station) => {
            ...
            if ((!lat || !lng) && station.site_id && station.site_id.location) {
                ...
                if (station.site_id.address)
                    station.location.address = station.site_id.address;
            }
            if (!lat || !lng)
                return null;
            const R = 6371e3;
            ...
            const Δλ = (lng - parseFloat(queryLng)) * Math.PI / 180;
...
function isPointNearPolyline(point, polyline, threshold) {
