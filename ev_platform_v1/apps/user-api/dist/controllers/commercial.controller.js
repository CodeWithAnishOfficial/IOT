    ...
    static async addCharger(req, res) {
        ...
        try {
            ...
            if (!charger_id || !price_per_kwh) {
                return res.status(400).json({ message: 'charger_id and price_per_kwh are required' });
            }
            // Check if charger exists
            const existing = await shared_1.Charger.findOne({ charger_id });
            if (existing) {
                return res.status(400).json({ message: 'Charger ID already exists' });
            }
            ...
            return res.status(201).json({ message: 'Charger added successfully', charger, site: newSite });
        }
        catch (error) {
