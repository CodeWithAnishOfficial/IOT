...
import { Charger, ChargingSession, WalletTransaction, Logger, Site } from '@ev-platform-v1/shared';

const logger = new Logger('CommercialController');

export class CommercialController {
  
  // Add a charger for commercialization
  static async addCharger(req: Request, res: Response) {
    ...
    try {
      ...
      if (createdSiteId) {
        try {
            await Site.findByIdAndDelete(createdSiteId);
  ...
  static async getMyChargers(req: Request, res: Response) {
    try {
      const userId = (req as any).user.user_id;
      // Populate site details
      const chargers = await Charger.find({ owner_id: userId }).populate('site_id');
      return res.json({
        error: false,
  ...
  static async getAnalytics(req: Request, res: Response) {
    try {
      ...
      const chargers = await Charger.find({ owner_id: userId }).select('charger_id');
      ...
      const sessions = await ChargingSession.find({ charger_id: { $in: chargerIds } })
        .sort({ created_date: -1 })
        .limit(100);
