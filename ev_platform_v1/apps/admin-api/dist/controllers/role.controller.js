...
const logger = new shared_1.Logger('RoleController');
class RoleController {
    static async getAllRoles(req, res) {
        try {
            const roles = await shared_1.Role.find().sort({ role_id: 1 });
    ...
    static async createRole(req, res) {
        try {
            ...
            const existingRole = await shared_1.Role.findOne({ role_name });
            if (existingRole) {
                return res.status(400).json({ error: true, message: 'Role Name already exists' });
            }
            // Auto-increment role_id
            const lastRole = await shared_1.Role.findOne().sort({ role_id: -1 });
            const role_id = lastRole ? lastRole.role_id + 1 : 1;
            const role = await shared_1.Role.create({
                role_id,
                role_name,
                description
            });
    ...
    static async updateRole(req, res) {
        try {
            ...
            const role = await shared_1.Role.findOneAndUpdate({ role_id: parseInt(id) }, { $set: updateData }, { new: true });
            if (!role)
    ...
    static async deleteRole(req, res) {
        try {
            ...
            const role = await shared_1.Role.findOneAndDelete({ role_id: parseInt(id) });
            if (!role)
                return res.status(404).json({ error: true, message: 'Role not found' });
