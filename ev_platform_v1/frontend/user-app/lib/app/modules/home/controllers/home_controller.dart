...
class HomeController extends GetxController {
  ...
  final isProgrammaticMapMove =
      false.obs; // Flag to prevent UI hiding during animation
    ...
    if (station != null) {
      ...
      if (station.location != null && _googleMapController != null) {
        try {
          ...
          print("Error animating camera in showStationDetails: $e");
        }
      }
  ...
  void navigateToStationDetails(Charger station) {
    selectedStation.value = station; // Ensure it's selected
    Get.to(
      () => StationDetailsView(station: station, controller: this),
  ...
  Future<void> launchMaps(Charger station) async {
  ...
  Future<void> _safeAnimateCamera(CameraUpdate update) async {
    try {
      isProgrammaticMapMove.value = true;
      ...
      if (routePoints.isNotEmpty) {
        ...
        tripPolylines.add(
          Polyline(
            ...
            points: routePoints,
            color: AppColors.primary.withOpacity(0.2), // Primary theme corridor
            width: 20, // Wide
            zIndex: 1,
        ...
        tripPolylines.add(
          Polyline(
            ...
            points: routePoints,
            color: AppColors.primary, // Use brand color (Fluorescent)
            width: 5,
            zIndex: 2,
            jointType: JointType.round,
            startCap: Cap.roundCap,
  ...
  void updateMapStyle(bool isDark) {
    // If controllers are not ready, do nothing
    if (_googleMapController == null && tripMapController == null) return;
    ...
    void applyStyle(String? style) {
      if (_googleMapController != null) {
        try {
          ...
          print("Ignored error updating home map style: $e");
        }
      }

      if (tripMapController != null) {
        try {
          tripMapController!.setMapStyle(style);
        } catch (e) {
          print("Ignored error updating trip map style: $e");
        }
      }
    }
    ...
    if (isDark) {
      if (_darkMapStyle != null) {
        applyStyle(_darkMapStyle);
      } else {
        // Load it if not loaded (fallback)
        rootBundle
            .loadString('assets/map_styles/dark_map_style.json')
            .then((style) {
              _darkMapStyle = style;
      ...
      if (_lightMapStyle != null) {
        ...
        rootBundle
            ...
            .catchError((e) {
              print("Error loading light map style: $e");
              applyStyle("[]"); // Fallback
            });
      }
    }
  ...
  void onMapCreated(GoogleMapController controller) {
    ...
    updateMapStyle(Get.isDarkMode);

    // Initial check for map ready
  ...
  void onTripMapCreated(GoogleMapController controller) {
    ...
    if (currentLocation.value != null) {
      try {
        ...
        print("Error animating trip map: $e");
      }
    }
  }
  ...
  Future<LatLngBounds?> prepareRoute(Charger station) async {
    ...
    // 1. Prioritize searched location (Red Pin) if available
    if (_selectedLocationMarker != null) {
      startPosition = _selectedLocationMarker!.position;
    }
  ...
  Future<void> startNavigation(Charger station) async {
    final bounds = await prepareRoute(station);
    if (bounds != null) {
      _safeAnimateCamera(CameraUpdate.newLatLngBounds(bounds, 50));
      currentSheetHeight.value = 0.28;
      _sheetAnimationController.add(0.28);
    } else {
      Get.snackbar("Error", "Failed to calculate route");
    }
  }
